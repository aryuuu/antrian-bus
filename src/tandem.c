...
#include "lib/simlib.h" /* Required for use of simlib.c. */
...
double mean_interarrival, mean_service_1, mean_service_2, min_transit,
    max_transit, time_end;
...
void arrive_1() /* Arrival event function. */
{
  ...
  if (list_size[LIST_SERVER_1] == 1) {
    ...
    event_schedule(sim_time + expon(mean_service_1, STREAM_SERVICE_1),
                   EVENT_DEPARTURE_1);
  }
}
...
void depart_1(void) /* Departure event function. */
{
  ...
  event_schedule(sim_time + uniform(min_transit, max_transit, STREAM_TRANSIT),
                 EVENT_ARRIVAL_2);
}
...
void arrive_2() /* Arrival event function. */
{
  ...
  if (list_size[LIST_SERVER_2] == 1) {
    ...
    /* Schedule a departure (service completion). */
    event_schedule(sim_time + expon(mean_service_2, STREAM_SERVICE_2),
                   EVENT_DEPARTURE_2);
  }
}
